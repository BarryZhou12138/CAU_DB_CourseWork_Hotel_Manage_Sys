...
def update_order_by_staff(username, password, order_id, check_in, check_out,new_room_type, connecter):
    print("update_order_by_staff "+username+" "+password+" "+order_id+" "+check_in+" "+check_out+" "+new_room_type)
    ...
    if len(search_result)==1 :
        if search_result[0][3]==password:
            ...
            search_usernames=connecter.search('用户表',"用户ID='%s'"%search_order_id[0][1])
            if len(search_usernames)==0:
                return False,'用户ID输入有误'
            usernames=search_usernames[0][1]
            passwords=search_usernames[0][9]
            return_information=add_new_order(usernames,passwords,new_room_type,check_in,check_out,connecter)
