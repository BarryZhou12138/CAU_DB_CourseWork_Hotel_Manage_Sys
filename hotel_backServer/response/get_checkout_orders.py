...
def get_checkout_orders(date, username, password, connecter):
    print("get_checkout_orders " + date + " " + username + " " + password)
    search_result = connecter.search('服务人员表', "服务人员联系电话='%s'" % username)
    if len(search_result) == 1:
        if password == search_result[0][3]:
            search_all_orders = connecter.search('订单表', "预计离开日期<='%s'" % date)
            ...
            result_order_ids = all_order_ids - out_order_ids 
