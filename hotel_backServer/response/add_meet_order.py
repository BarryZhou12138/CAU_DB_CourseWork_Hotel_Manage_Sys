...
def add_meet_order(username, password, meet_id, meet_time,location,connecter):
    print("add_meet_order "+username+" "+password+" "+meet_id+" "+meet_time+" "+location)
    ...
    if len(search_result) == 1:
        if search_result[0][9] == password:
            if n>= 1:
                 ...
                 connecter.insert('接站预约表(接站订单编号,接送形式编码,订单编号,接站日期,接站地址)',"('%s','%s','%s','%s','%s')"%(meet_order_id,meet_id,search_order_id[n-1][0],meet_time,location))
                 result_data.append(
                    {
